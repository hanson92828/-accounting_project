...
<body>
    ...
    <?php
        ...
        $sql = "SELECT cid,cname,year,month,product,amount,unit,oid,unit_price from myorder ORDER BY oid DESC";
        $show_all_order=[];
        ...
        if ($result = mysqli_query($conn,$sql))
        {
            ...
            {
                    array_push($show_all_order,[]);
                    array_push($show_all_order[$count],$row[0],$row[1],$row[2],$row[3], $row[4], $row[5], $row[6], $row[7], $row[8]);
        ...
        <form action="searchOrder.php" method="post">
            <div class="row g-1">
                ...
                <div class="col-lg-2" style="display: flex; align-items: center;">
                    <label for="formGroupExampleInput" class="form-label">年分/月份</label>
        ...
        <table style="border: 3px #cccccc solid;" rules="all" cellpadding="20">
            <?php
                ...
                else
                {
                    ...
                    echo "<td class='col-1' style='text-align: center; border-width:1px;'></td>";
                    echo "</tr>";
                    for ($i = 0; $i<count($show_all_order); $i++)
                    {
                        echo "<tr>";
                        echo "<td class='col-1' style='text-align: center; border-width:1px;'>".$show_all_order[$i][0]."</td>";
                        echo "<td class='col-1' style='text-align: center; border-width:1px;'>".$show_all_order[$i][1]."</td>";
                        echo "<td class='col-1' style='text-align: center; border-width:1px;'>".$show_all_order[$i][2]."/".$show_all_order[$i][3]."</td>";
                        echo "<td class='col-2' style='text-align: center; border-width:1px;'>".$show_all_order[$i][4]."</td>";
                        echo "<td class='col-1' style='text-align: center; border-width:1px;'>".$show_all_order[$i][5]."</td>";
                        echo "<td class='col-1' style='text-align: center; border-width:1px;'>".$show_all_order[$i][6]."</td>";
                        echo "<td class='col-1' style='text-align: center; border-width:1px;'>".$show_all_order[$i][8]."</td>";
                        echo "<td class='col-1' style='text-align: center; border-width:1px;'>"."<a class='topic' href='updateOrder.php?oid=".$show_all_order[$i][7]."' style='font-size: 15pt;font-weight: bold;'>修改</a>"."</td>";
                        echo "<td class='col-1' style='text-align: center; border-width:1px;'>"."<a class='topic' href='deleteOrder.php?oid=".$show_all_order[$i][7]."' style='font-size: 15pt;font-weight: bold;'>刪除</a>"."</td>";
                        echo "</tr>";
